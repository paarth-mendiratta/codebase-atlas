  ...
  const codemapContent = useMemo(() => {
    ...
    const fileRows = files
      .map((f) => {
        ...
        return `| \`${f.path}\` | ${cleanedExp} | ${f.size} bytes |`;
      })
      .join("\n");
...
`;
  }, [repoName, overview, files, explanations, dependencyMapStr]);

  // Token savings calculations
  const { fullRepoTokens, mapTokens, tokenReductionPercent } = useMemo(() => {
    // Sum of file sizes or totalContentBytes / 4
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-6 space-y-6 overflow-y-auto">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <button
                ...
                className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium rounded-xl border border-slate-700 transition-colors flex items-center justify-between gap-2"
              >
          ...
          <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800 flex items-center justify-between text-xs">
            ...
            <button
              ...
              disabled={rechecking}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg border border-slate-700 transition-colors text-xs font-medium disabled:opacity-50"
            >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-800 bg-slate-900/80">
          <button
            ...
            className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium rounded-xl border border-slate-700 transition-colors flex items-center gap-2"
          >
            {copiedType === "raw" ? (
              <>
          ...
          <button
            ...
            className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-medium rounded-xl transition-colors shadow-lg shadow-indigo-600/20 flex items-center gap-2"
          >
